...
export async function fetchPointsConfig() {
	return fetchPublic<{ text_to_image: number; image_to_image: number; llm_translate: number; signup_bonus: number; text_to_image_anima: number; tts_generate: number }>('/api/wallet/points-config');
}
